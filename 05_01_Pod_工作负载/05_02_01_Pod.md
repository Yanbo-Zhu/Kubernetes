...
| 功能                        | 说明                                                                        |
| ------------------------- | ------------------------------------------------------------------------- |
| ✅ **支持多种匹配方式**            | `In`, `NotIn`, `Exists`, `DoesNotExist`, `Gt`, `Lt`                       |
| ✅ **支持逻辑组合**              | 多个 key-value 条件组合                                                         |
| ✅ **支持软/硬约束**             | 分为 `requiredDuringScheduling...`（必须） 和 `preferredDuringScheduling...`（尽量） |
| ✅ **兼容多个值**               | 比如多个实例类型、区域等                                                              |
| ❌ 相比 `nodeSelector` 更复杂一点 |                                                                           |
...
### 7.2.2 nodeAntiAffinity 

ubernetes 中 **没有** `nodeAntiAffinity` 这个字段，**不支持以“排除某些节点”为目的的反向 Node Affinity**。  
如果你想避免调度到某些 Node，正确的方法是使用：

---

方式 1：`nodeAffinity` + `NotIn`
```
spec:
  affinity:
    nodeAffinity:
...
这个写法可以实现类似 “不要调度到标签为 `disktype=hdd` 的节点”。


---

方式 2：使用 `taints` + `tolerations` 控制调度拒绝
如果你想明确“排斥某类节点”，推荐方式是：
1. 给不希望调度的节点打上 **污点（Taint）**
2. 没有容忍（toleration）这个污点的 Pod 就无法被调度到上面
```
...
以下是一个 nodeAntiAffinity 的 完整示例，它表示：不要将 Pod 调度到已经运行同一 app 的节点上（防止多个副本落在同一节点，提高可用性）。

避免将相同的 app 部署在同一个节点上
